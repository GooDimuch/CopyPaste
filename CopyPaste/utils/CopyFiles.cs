...
namespace CopyPaste.utils {
	...
	public class CopyFiles {
		...
		// Delegates
		private delegate CopyProgressResult CopyProgressRoutine(Int64 TotalFileSize, Int64 TotalBytesTransferred,
																														Int64 StreamSize, Int64 StreamBytesTransferred, UInt32 dwStreamNumber,
																														CopyProgressCallbackReason dwCallbackReason, IntPtr hSourceFile, IntPtr hDestinationFile,
																														IntPtr lpData);

		private delegate CopyProgressResult DEL_CopyProgressHandler(Int64 total, Int64 transferred, Int64 streamSize,
																																Int64 StreamByteTrans, UInt32 dwStreamNumber, CopyProgressCallbackReason reason,
		...
		static extern unsafe bool CopyFileEx(string lpExistingFileName, string lpNewFileName,
																				CopyProgressRoutine lpProgressRoutine, IntPtr lpData, Boolean* pbCancel, CopyFileFlags dwCopyFlags);
		...
		private List<String> GetFiles(String sourceDir) {
			...
			foreach (String filename in fileEntries) { foundFiles.Add(filename); }
			...
			foreach (string subdir in subdirEntries) {
				...
				if ((System.IO.File.GetAttributes(subdir) & System.IO.FileAttributes.ReparsePoint) !=
		...
		private CopyProgressResult CopyProgressHandler(Int64 total, Int64 transferred, Int64 streamSize,
																									Int64 StreamByteTrans, UInt32 dwStreamNumber, CopyProgressCallbackReason reason,
																									IntPtr hSourceFile, IntPtr hDestinationFile, IntPtr lpData) {
			//Check to see if there is a dialog window to use
			if (digWindow != null) {
				//Are we going to send the update on the correct thread?
				if (digWindow.SynchronizationObject != null &&
						digWindow.SynchronizationObject.InvokeRequired) {
				...
				} else { digWindow.update(totalFiles, totalFilesCopied, total, transferred, currentFilename); }
			}
		...
		private void ShowDiag(ICopyFilesDiag diag) {
			...
			if (digWindow != null) {
				...
				if (digWindow.SynchronizationObject != null &&
						digWindow.SynchronizationObject.InvokeRequired) {
					digWindow.SynchronizationObject.Invoke(new DEL_ShowDiag(ShowDiag), new Object[] {diag});
				} else { diag.Show(); }
			}
		}
		...
		private void HideDiag(ICopyFilesDiag diag) {
			...
			if (digWindow != null) {
				...
				if (digWindow.SynchronizationObject != null &&
						digWindow.SynchronizationObject.InvokeRequired) {
					digWindow.SynchronizationObject.Invoke(new DEL_HideDiag(HideDiag), new Object[] {diag});
				} else {
					diag.Hide();
					cancel = false;
				}
		...
		private void Copyfiles() {
			...
			if (sourceDir != "") { files = GetFiles(sourceDir); } else { CheckFilenames(); }
			totalFiles = files.Count;
			...
			foreach (String filename in files.ToArray()) {
				...
				else { tempFilepath = System.IO.Path.Combine(destinationDir, newFilenames[index]); }
				...
				for (int i = 0; i < filepath.Length - 1; i++) { tempDirPath += filepath[i] + "\\"; }
				if (!System.IO.Directory.Exists(tempDirPath)) { System.IO.Directory.CreateDirectory(tempDirPath); }
		...
		private void OnCopyComplete() {
			if (EV_copyComplete != null) { EV_copyComplete(); }
		}

		private void OnCopyCanceled() {
			if (EV_copyCanceled != null) { EV_copyCanceled(filesCopied); }
		}
					...
					if (innerIndex != index) {
						if (originalFilename == dupeFilename) {
							...
							foreach (String newFilename in tempFileNameArr) {
								...
								if (filenameIndex == tempFileNameArr.Count - 1) { fileNames[innerIndex] += "."; }
		...
		//Copy the files
		public void Copy() { Copyfiles(); }

		public void CopyAsync(ICopyFilesDiag diag) {
			digWindow = diag;

			if (digWindow != null &&
					digWindow.SynchronizationObject == null) {
